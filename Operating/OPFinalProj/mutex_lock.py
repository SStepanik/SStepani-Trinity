...
class ProcessManager:
    ...
    def add_process(self, name, length):
        with self.mutex:
            ...
            print(Fore.WHITE + f'Process {name} ({length} seconds) added to the queue.' + Style.RESET_ALL)
    ...
    def run_processes(self):
        while True:
            ...
            print(Fore.YELLOW + f'Running process {name} ({length} seconds)...' + Style.RESET_ALL)
            time.sleep(length)
